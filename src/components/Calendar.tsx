...
const WeekView = () => {
  ...
  const generateRowData = () => {
    ...
    events.forEach((event) => {
      ...
      // Loop through each day of the week and check if the event should be displayed
      rowData.forEach((row) => {
        let startCol = startDate.clone().isoWeekday() - 1; // Start column index
        let endCol = endDate.clone().isoWeekday() - 1; // End column index

        if (
          endDate.isBefore(
            currentWeek.clone().startOf('isoWeek').add(6, 'days')
          )
        ) {
          endCol = endDate.clone().isoWeekday() - 1; // Adjust if end date is within the week
        } else {
          endCol = 6; // Cap at Sunday if it goes beyond the week
        }
        ...
        for (let i = startCol; i <= endCol; i++) {
          const dayKey = moment()
            ...
            .format('dddd')
            .toLowerCase();
          const dayStart = moment(currentWeek).add(i, 'days').startOf('day');
          const dayEnd = moment(currentWeek).add(i, 'days').endOf('day');

          if (
            dayEnd.isSameOrAfter(startDate) &&
            dayStart.isSameOrBefore(endDate)
          ) {
            row[dayKey] = (
